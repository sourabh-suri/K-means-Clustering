...
int main(int argc, char** argv){
   ...
   // start=MPI_Wtime();
	...
	int cold_r[K], cold_g[K], cold_b[K];
    	...
    	int iter=0;
    	int converge=0;int thr=0;
    	/*******************************************looping********************************************/
    	while( converge !=1)
    	{
    	        iter++;
		for(int j=0 ; j< K; j++){
	    	cluster_sum_r[j]=0; cluster_sum_g[j]=0; cluster_sum_b[j]=0;
	    	cluster_count[j]=0;
	    	count[j]=0; thr=0;
	    	sum_r[j]=0; sum_g[j]=0; sum_b[j]=0;
	    	cold_r[j]=centroid_r[j]; cold_g[j]=centroid_g[j]; cold_b[j]=centroid_b[j];	    	
       	}
    	...
    	//MPI_Send(&converge, 1, MPI_INT, proc,211, MPI_COMM_WORLD);/**********/
    	}
 	  printf("iter: %d\n",iter);  	
	...
	//	printf(" fsd: %d %d %d\n",i, cluster_sum_r[i], cluster_count[i]);
    	 ...
    	 for(int i=0;i<K;i++)
    	 thr+= abs(centroid_r[i]-cold_r[i])+abs(centroid_g[i]-cold_g[i])+abs(centroid_b[i]-cold_b[i]);
       	...
       	int c=0;
	    	...
	    	for(int j=0;j<width;j++){	    	
	    		...
	    		//printf(" c: %d %d %d %d\n",c,my_rank ,slave_sum_r[c],slave_count[c]);
	    	 	...
	    	 	//printf(" after:%d %d  %d\n",my_rank, slave_sum_r[i],  slave_count[i]);
	    	       MPI_Recv(&con, 1, MPI_INT, 0, 211, MPI_COMM_WORLD ,&stat);
	    	// if(my_rank == 1)	    	  //  MPI_Barrier(MPI_COMM_WORLD);
		//	for(int j=0 ; j< K; j++)
	    }	 
